...
public static class CommandService
{
        ...
        Player player)
    {
        ...
        {
            GwServerPlugin.Logger.LogWarning($"Player {player.GetDisplayName()} does not have permission to execute command {command.Name}");
        ...
        if (await command.Validate(player, args))
        {
            ...
            if (executionResult.success)
            {
                GwServerPlugin.Logger.LogInfo(
                    $"Command {command.Name} executed successfully by {player.GetDisplayName()} with argument(s): {string.Join(", ", args)}"
                );
                return (true, response);
            }

            GwServerPlugin.Logger.LogWarning(
                $"Failed to execute command {command.Name} by {player.GetDisplayName()} with argument(s): {string.Join(", ", args)}");
            response ??= $"Failed to execute command {command.Name}";
            return (false, response);
        }

        GwServerPlugin.Logger.LogInfo(
            $"Failed validation for command {command.Name} by {player.GetDisplayName()} with argument(s): {string.Join(", ", args)}");
